...
namespace OneNorth.ExpressSubitem.FieldTypes
{
  public class ExpressSubitemField : Input, IContentField
  {
    ...
    /// <returns></returns>
    private string BuildDropdownOptions(Item subItem, Field field, out bool selectionInList)
    {
      var fieldChoices = GetSources(subItem, field);
      ...
      //Special case where none or string is empty.
      if (!selectionInList) selectionInList = string.IsNullOrEmpty(field.Value);
